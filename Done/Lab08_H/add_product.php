...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['name'], $_POST['price'], $_POST['desc'], $_FILES['image'])) {
        $name = $_POST['name'];
        $price = $_POST['price'];
        $desc = $_POST['desc'];
        ...
        else if (intval($price) <= 0 || intval($price) < 1000000 || intval($price) % 10000 != 0) {
        ...
        // Kiểm tra xem tệp đã được tải lên không
            ...
            $stmt->bind_param("siss", $name, $price, $desc, $image_name);

            // Thực thi statement
            if ($stmt->execute()) {
                header("Location: index.php");
                exit(); // Chắc chắn rằng không có mã HTML hoặc mã PHP khác được gửi sau chuyển hướng
            } else {
            ...
            <div class="col-xl-5 col-lg-6 col-md-8 border rounded my-5 p-4  mx-3">
                ...
                <form method="post" action="" novalidate enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input value="<?= $price ?>" name="price" required class="form-control" type="number"
                            placeholder="Giá bán" id="price">
                    </div>
                    <div class="form-group">
